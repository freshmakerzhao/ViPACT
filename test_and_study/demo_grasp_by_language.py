...
def save_mask_debug(rgb: np.ndarray, mask: np.ndarray, output_prefix: str) -> Dict[str, str]:
    mask = mask.astype(np.float32)
    mask_path = f"{output_prefix}_mask.png"
    overlay_path = f"{output_prefix}_overlay.png"
    plt.imsave(mask_path, mask, cmap="gray", vmin=0.0, vmax=1.0)
    overlay = rgb.astype(np.float32).copy()
    active = mask > 0.5
    overlay[active] = 0.65 * overlay[active] + 0.35 * np.array([255, 0, 0], dtype=np.float32)
    overlay = np.clip(overlay, 0, 255).astype(np.uint8)
    plt.imsave(overlay_path, overlay)
    return {"mask": mask_path, "overlay": overlay_path}


def main():
    parser = argparse.ArgumentParser(
        description="Language -> mask (GroundingDINO+SAM) -> ViPACT ACT rollout."
    )
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--text-prompt", type=str, required=True, help='e.g. "the red block"')
    parser.add_argument("--episode-id", type=int, default=0)
    parser.add_argument("--output-dir", type=str, default="./notes/target_demo")
    parser.add_argument("--max-steps", type=int, default=-1)
    parser.add_argument("--save-video", action="store_true")
    parser.add_argument("--render", action="store_true")
    parser.add_argument("--expected-target-id", type=int, default=-1, help="Optional for strict success metric")
    parser.add_argument("--mask-camera", type=str, default=MASK_CAMERA)
    ...
    if args.mask_camera not in camera_names:
        raise ValueError(f"--mask-camera={args.mask_camera} not in camera_names={camera_names}")
    debug_camera = args.mask_camera
    ...
    BOX_POSE[0] = np.asarray(sample_complex_scene_pose_eval(), dtype=np.float64).copy()
    env = make_sim_env(task_name, equipment_model=equipment_model)
    ts = env.reset()
    target_geom_names = list_target_geom_names(env._physics)

    # High-level brain call once on first frame (static mask for this episode)
    ...
    first_rgb = ts.observation["images"][args.mask_camera]
    predicted_mask_u8, mask_debug = pipeline.predict_mask_with_debug(first_rgb, args.text_prompt)
    predicted_mask = predicted_mask_u8.astype(np.float32)
    static_mask_dict = {args.mask_camera: predicted_mask}
    mask_paths = save_mask_debug(first_rgb, predicted_mask, str(output_dir / f"lang_ep{args.episode_id}_{args.mask_camera}"))

    if args.expected_target_id >= 0 and args.expected_target_id < len(target_geom_names):
        expected_target_geom = target_geom_names[args.expected_target_id]
        if hasattr(env, "task"):
            env.task.current_target_id = int(args.expected_target_id)
            env.task.target_geom_name = expected_target_geom
    else:
        expected_target_geom = None
    ...
    for t in range(episode_len):
        ...
        step_output = runtime.step(
            PolicyStepInput(
                ...
                brain_output=BrainOutput(target_id=int(args.expected_target_id) if args.expected_target_id >= 0 else 0),
    ...
    video_path = ""
    if args.save_video:
        video_path = str(output_dir / f"lang_demo_ep{args.episode_id}.mp4")
    ...
    target_lifted = bool(lifted_flags[expected_target_geom]) if expected_target_geom is not None else None
    strict_success = bool(target_lifted and guessed_grab_geom == expected_target_geom) if expected_target_geom is not None else None
    ...
    summary = {
        ...
        "stats_path": stats_path,
        "camera_names": camera_names,
        "mask_camera_names": [args.mask_camera],
        "mask_debug": mask_debug,
        "mask_paths": mask_paths,
        "expected_target_id": int(args.expected_target_id),
        "expected_target_geom": expected_target_geom,
    ...
    summary_path = output_dir / f"summary_lang_demo_ep{args.episode_id}.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print(f"Saved summary: {summary_path}")
    print(f"Mask debug: {mask_paths}")
    if video_path:
