...
def build_policy_image(ts, camera_names: List[str], device: torch.device, static_mask_dict: Dict[str, np.ndarray]):
    curr_images = []
    active_mask_cameras = {MASK_CAMERA} if MASK_CAMERA in camera_names else set()
    for cam_name in camera_names:
        rgb = rearrange(ts.observation["images"][cam_name], "h w c -> c h w").astype(np.float32) / 255.0
        if cam_name in active_mask_cameras:
            if cam_name not in static_mask_dict:
                raise ValueError(f"Missing oracle mask for active camera={cam_name}")
            mask_2d = static_mask_dict[cam_name].astype(np.float32)
        else:
...
def parse_target_ids(target_spec: str, max_count: int) -> List[int]:
    target_spec = (target_spec or "all").strip().lower()
    if target_spec == "all":
    ...
    for part in target_spec.split(","):
        ...
        if not part:
            continue
        target_id = int(part)
        if target_id < 0 or target_id >= max_count:
            raise ValueError(f"target id {target_id} out of range [0, {max_count - 1}]")
        if target_id not in ids:
            ids.append(target_id)
    if len(ids) == 0:
        raise ValueError("No valid target ids parsed from --target-ids")
    return ids
...
def save_debug_images(
    ...
    numbered_path = os.path.join(output_dir, f"{prefix}{debug_camera}_numbered_targets.png")
    fig.tight_layout()
    fig.savefig(numbered_path, dpi=120, bbox_inches="tight", pad_inches=0)
    plt.close(fig)

    mask_path = os.path.join(output_dir, f"{prefix}{debug_camera}_target_mask.png")
    ...
    overlay_path = os.path.join(output_dir, f"{prefix}{debug_camera}_oracle_overlay.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="Target-conditioned demo (fixed: ACT + fairino5_single + complex lifting)."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help='Target ids: e.g. "all" (default), "0", or "0,1,2,3"',
    )
    ...
    parser.add_argument("--render", action="store_true", help="Enable on-screen render (default off)")
    parser.add_argument("--no-save-video", action="store_true", help="Do not save rollout videos")
    ...
    debug_camera = MASK_CAMERA if MASK_CAMERA in camera_names else camera_names[0]
    if debug_camera != MASK_CAMERA:
        print(f"[WARN] camera '{MASK_CAMERA}' not found. Debug camera fallback to '{debug_camera}'.")
    ...
    # 固定一个布局，多个 target_id 在同场景下切换，便于验证 “指哪打哪”
    base_init_pose = np.asarray(sample_complex_scene_pose_eval(), dtype=np.float64).copy()
    BOX_POSE[0] = base_init_pose.copy()
    probe_env = make_sim_env(task_name, equipment_model=equipment_model)
    _ = probe_env.reset()
    target_geom_names = list_target_geom_names(probe_env._physics)
    del probe_env
    print("Target id mapping:")
    for i, name in enumerate(target_geom_names):
        print(f"  [{i}] {name}")
    selected_target_ids = parse_target_ids(args.target_ids, len(target_geom_names))
    print(f"Selected target ids: {selected_target_ids}")
    ...
    for target_id in selected_target_ids:
        selected_target_geom = target_geom_names[target_id]
        BOX_POSE[0] = base_init_pose.copy()
        env = make_sim_env(task_name, equipment_model=equipment_model)
        ts = env.reset()

        mask_camera_names = [MASK_CAMERA] if MASK_CAMERA in camera_names else []
        static_mask_dict = build_mask_for_single_geom(env._physics, mask_camera_names, selected_target_geom)
        static_mask_dict = apply_mask_ablation(static_mask_dict, mode=DEFAULT_MASK_ABLATION, seed=seed)

        file_prefix = f"target_{target_id}_{selected_target_geom}_seed_{seed}"
        debug_mask = static_mask_dict.get(
            debug_camera,
            np.zeros(ts.observation["images"][debug_camera].shape[:2], dtype=np.float32),
        )
        debug_paths = save_debug_images(
            physics=env._physics,
            rgb_image=ts.observation["images"][debug_camera],
            debug_camera=debug_camera,
            target_geom_names=target_geom_names,
            selected_target_geom=selected_target_geom,
            target_mask=debug_mask,
        ...
        if args.render:
            ax = plt.subplot()
            plt_img = ax.imshow(ts.observation["images"][debug_camera])
            plt.ion()

        with torch.inference_mode():
            for t in range(episode_len):
                if args.render:
                    plt_img.set_data(ts.observation["images"][debug_camera])
                    plt.pause(DT)
        ...
        if save_video:
            video_path = os.path.join(
                output_dir,
                f"demo_target_{target_id}_{selected_target_geom}_seed_{seed}.mp4",
            )
        ...
        print(f"mask_ablation: {DEFAULT_MASK_ABLATION}")
        ...
        summary = {
            ...
            "mask_ablation": DEFAULT_MASK_ABLATION,
        ...
        summary_path = os.path.join(
            output_dir,
            f"summary_target_{target_id}_{selected_target_geom}_seed_{seed}.json",
        )
    ...
    agg_path = os.path.join(output_dir, f"summary_all_targets_seed_{seed}.json")
    strict_success = 0
    for s in all_summaries:
        strict_success += int(bool(s["target_lifted"]) and s["guessed_grab_geom"] == s["target_geom"])
    agg = {
        "seed": seed,
        "selected_target_ids": selected_target_ids,
        "count": len(all_summaries),
        "strict_success": strict_success,
        "strict_success_rate": float(strict_success / len(all_summaries)) if all_summaries else 0.0,
