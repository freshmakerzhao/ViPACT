...
conda run -n act3_env python record_sim_episodes.py --yaml config \
  --config_path configs/fairino5_single_26041702_with_complex_scene_cf_mask_longrun/01_record.yaml
```

录制完成后训练：
```bash
conda run -n act3_env python imitate_episodes.py --yaml config \
  --config_path configs/fairino5_single_26041702_with_complex_scene_cf_mask_longrun/02_train.yaml
```

训练完成后评估：
```bash
conda run -n act3_env python imitate_episodes.py --yaml config \
  --config_path configs/fairino5_single_26041702_with_complex_scene_cf_mask_longrun/03_eval.yaml \
  --eval
```
